...
-define(TRANSFORM_FUNCTIONS,
        [{{is_map_key, 2}, {maps, is_key}},
         ...
         {{[pid_to_binary, port_to_binary, ref_to_binary], 1}, otpbp_erlang},
         ...
         {{gb_sets, [filtermap, is_equal, iterator, larger, map, smaller], 2}, otp_gb_sets},
         {{gb_sets, iterator_from, 3}, otp_gb_sets},
         ...
         {{graph, add_edge, [3, 4]}, otpbp_graph},
         {{graph, [del_edges, has_edge, vertex], [2, 3]}, otpbp_graph},
         {{graph, [add_vertex, edges], [1, 2, 3]}, otpbp_graph},
         {{graph,
           [del_edge, del_vertex, del_vertices,
            get_cycle, get_short_cycle,
            has_vertex,
            in_degree, in_edges, in_neighbours,
            out_degree, out_edges, out_neighbours],
           2},
          otpbp_graph},
         {{graph, [del_path, fold_vertices, get_path, get_short_path, has_path], 3}, otpbp_graph},
         {{graph, [info, no_edges, no_vertices, sink_vertices, source_vertices, vertices, vertices_with_labels], 1},
          otpbp_graph},
         {{graph, new, [0, 1]}, otpbp_graph},
         {{graph,
           [arborescence_root,
            components, condensation, cyclic_strong_components,
            is_acyclic, is_arborescence, is_tree,
            loop_vertices,
            roots,
            strong_components,
            topsort],
           1},
          otpbp_graph},
         {{graph, [preorder, postorder, reverse_postorder], [1, 2]}, otpbp_graph},
         {{graph, [reachable, reachable_via_neighbours, reaching, reaching_via_neighbours], 2}, otpbp_graph},
         {{graph, subgraph, [2, 3]}, otpbp_graph},
         ...
         {{rand, [bytes, shuffle], 1}, otpbp_rand},
         {{rand, [bytes_s, shuffle_s], 2}, otpbp_rand},
         ...
         {{scheduler, [get_sample, get_sample_all], 0}, otpbp_scheduler},
         ...
         {{uri_string, [percent_decode, unquote], 1}, otpbp_uri_string},
